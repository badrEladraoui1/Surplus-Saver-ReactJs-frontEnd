...
const Accepted = () => {
    ...
    const fetchAcceptedInterests = async () => {
      try {
        const response = await axios.get(
          `${api}/SurplusSaverApiV1/interests/user/consumer/accepted`,
          {
            headers: {
        ...
        setAcceptedInterests(response.data);
      } catch (error) {
        console.error("Failed to fetch accepted interests:", error);
      }
    };
  ...
  const memoizedAcceptedInterests = useMemo(
    () => acceptedInterests,
    [acceptedInterests]
  );
  ...
  return (
    <div className="flex flex-col gap-10">
      {memoizedAcceptedInterests.map((interest) => (
        <div
          key={interest.id}
        ...
        >
          ...
          <p className="text-2xl font-bold text-center">
            <span>FOR MORE INFO</span> , contact the restaurant at this phone
            number:{" "}
            <TextShine
